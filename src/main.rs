...
use args::MandelbrotArgs;
use clap::Parser;
use mandelbrot::Mandelbrot;
use std::process;

fn main() {
    let args = MandelbrotArgs::parse();

    let domain: Vec<f64> = args.x.split(",").map(|s| s.parse().unwrap()).collect();
    if domain.len() != 2 {
    ...
    let range: Vec<f64> = args.y.split(",").map(|s| s.parse().unwrap()).collect();
    if range.len() != 2 {
